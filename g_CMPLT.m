% g_CMPLT edited by ray 2017.5.10
%  all symbols in c_OFDM.m
...
for i = 1:6
    ...
    fs1 = fs1';
    k = (0:19)' ; 
    TableL = [num2str(400 + k * 4,  '%3d|') num2str(fs1(:,1), '%6.3f') ...
                num2str(400 + k * 4 + 1, '|%4d|') num2str(fs1(:,2), '%6.3f') ...
                num2str(400 + k * 4 + 2,    '|%4d|') num2str(fs1(:,3), '%6.3f') ...
                num2str(400 + k * 4 + 3, '|%4d|') num2str(fs1(:,4), '%6.3f') ];
    Tableans = [Tableans; TableL];
end
%  Plotting the first symbol in time
dt = 3.2/384 ;  % microseconds
t = (0:383)*dt ;
temp = real(SmblT(:,1));
for i = 2:6
    temp = [temp; real(SmblT(:,i))];
end
plot(t, temp), grid on
axis([0, 3.2, -0.2, 0.2])
title('All OFDM symbol in the time domain')
xlabel('time \mus')
